...
impl Rp1PcieTransport {
    ...
    fn log_watchdog_quiescence(&mut self, rp1: &arch_hal::soc::bcm2712::Rp1Config) {
        ...
        const VERSION: u32 = if KERNEL_RESTART {8} else if EXPIRY_ENTRY {7} else if SELFTEST {6} else if LATE_CONTROL { 5 } else if POSTACK { 4 } else { 3 };
        const MAGIC: u32 = u32::from_le_bytes(if KERNEL_RESTART {*b"WDT8"} else if EXPIRY_ENTRY {*b"WDT7"} else if SELFTEST {*b"WDT6"} else if LATE_CONTROL { *b"WDT5" } else if POSTACK { *b"WDT4" } else { *b"WDT3" });
        let mut request: [u32; 8] = [u32::from_le_bytes(if KERNEL_RESTART {*b"WQ08"} else if EXPIRY_ENTRY {*b"WQ07"} else if SELFTEST {*b"WQ06"} else if LATE_CONTROL { *b"WQ05" } else if POSTACK { *b"WQ04" } else { *b"WQ03" }),VERSION,1,1,LOAD,256,0,
            0x5744_5432 ^ VERSION ^ 1 ^ 1 ^ LOAD ^ 256];
        let mut ack: [u32; 8] = [u32::from_le_bytes(if KERNEL_RESTART {*b"QA08"} else if EXPIRY_ENTRY {*b"QA07"} else if SELFTEST {*b"QA06"} else if LATE_CONTROL { *b"QA05" } else if POSTACK { *b"QA04" } else { *b"QA03" }),VERSION,1,2,0,0,0,
