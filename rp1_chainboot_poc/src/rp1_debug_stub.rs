...
#[cfg(any(feature = "rp1-spi-peer-final-record", feature = "rp1-spi-rearm-final-record", feature = "rp1-spi-fifo-final-record", feature = "rp1-spi-varied-final-record", feature = "rp1-spi-retained-final-record", feature = "rp1-spi-overflow-final-record", feature = "rp1-spi-deadline-final-record", feature = "rp1-i2c-readonly-final-record", feature = "rp1-i2c-stop-final-record", feature = "rp1-i2c-read1-final-record"))]
const SPI_FINAL_BYTES: usize = if cfg!(feature = "rp1-i2c-read1-final-record") { 768 } else if cfg!(feature = "rp1-i2c-stop-final-record") { 512 } else if cfg!(feature = "rp1-i2c-readonly-final-record") { 160 } else if cfg!(feature = "rp1-spi-deadline-final-record") { 640 } else if cfg!(feature = "rp1-spi-overflow-final-record") { 768 } else if cfg!(feature = "rp1-spi-retained-final-record") { 448 } else if cfg!(feature = "rp1-spi-varied-final-record") { 448 } else if cfg!(feature = "rp1-spi-fifo-final-record") { 768 } else if cfg!(feature = "rp1-spi-rearm-final-record") { 208 } else { 128 };
#[cfg(any(feature = "rp1-spi-peer-final-record", feature = "rp1-spi-rearm-final-record", feature = "rp1-spi-fifo-final-record", feature = "rp1-spi-varied-final-record", feature = "rp1-spi-retained-final-record", feature = "rp1-spi-overflow-final-record", feature = "rp1-spi-deadline-final-record", feature = "rp1-i2c-readonly-final-record", feature = "rp1-i2c-stop-final-record", feature = "rp1-i2c-read1-final-record"))]
const SPI_FINAL_MAGIC: [u8; 4] = if cfg!(feature = "rp1-i2c-read1-final-record") { *b"I1X1" } else if cfg!(feature = "rp1-i2c-stop-final-record") { *b"I1S1" } else if cfg!(feature = "rp1-i2c-readonly-final-record") { *b"I1R1" } else if cfg!(feature = "rp1-spi-deadline-final-record") { *b"S0C2" } else if cfg!(feature = "rp1-spi-overflow-final-record") { *b"S0O2" } else if cfg!(feature = "rp1-spi-retained-final-record") { *b"S0K2" } else if cfg!(feature = "rp1-spi-varied-final-record") { *b"S0V2" } else if cfg!(feature = "rp1-spi-fifo-final-record") { *b"S0F2" } else if cfg!(feature = "rp1-spi-rearm-final-record") { *b"S0R2" } else { *b"S0P1" };
...
impl Rp1PcieTransport {
    ...
    fn log_watchdog_quiescence(&mut self, rp1: &arch_hal::soc::bcm2712::Rp1Config) {
        ...
        const WARM_GUARD: bool = cfg!(feature = "rp1-rtos-watchdog-warm-guard");
        const KERNEL_RESTART: bool = cfg!(feature = "rp1-rtos-watchdog-kernel-restart");
        const EXPIRY_ENTRY: bool = cfg!(feature = "rp1-rtos-watchdog-expiry-entry");
        const NONCE_LINKED: bool = SELFTEST || EXPIRY_ENTRY;
        const VERSION: u32 = if WARM_GUARD {9} else if KERNEL_RESTART {8} else if EXPIRY_ENTRY {7} else if SELFTEST {6} else if LATE_CONTROL { 5 } else if POSTACK { 4 } else { 3 };
        const MAGIC: u32 = u32::from_le_bytes(if WARM_GUARD {*b"WDT9"} else if KERNEL_RESTART {*b"WDT8"} else if EXPIRY_ENTRY {*b"WDT7"} else if SELFTEST {*b"WDT6"} else if LATE_CONTROL { *b"WDT5" } else if POSTACK { *b"WDT4" } else { *b"WDT3" });
        let mut request: [u32; 8] = [u32::from_le_bytes(if WARM_GUARD {*b"WQ09"} else if KERNEL_RESTART {*b"WQ08"} else if EXPIRY_ENTRY {*b"WQ07"} else if SELFTEST {*b"WQ06"} else if LATE_CONTROL { *b"WQ05" } else if POSTACK { *b"WQ04" } else { *b"WQ03" }),VERSION,1,1,LOAD,256,0,
            0x5744_5432 ^ VERSION ^ 1 ^ 1 ^ LOAD ^ 256];
        let mut ack: [u32; 8] = [u32::from_le_bytes(if WARM_GUARD {*b"QA09"} else if KERNEL_RESTART {*b"QA08"} else if EXPIRY_ENTRY {*b"QA07"} else if SELFTEST {*b"QA06"} else if LATE_CONTROL { *b"QA05" } else if POSTACK { *b"QA04" } else { *b"QA03" }),VERSION,1,2,0,0,0,
        ...
        for _ in 0..240 {
            ...
            if requested && w[98] == 4 {
                let valid = w[100] == 1 && w[103] == 1 && w[104..107] == [0,3,50] &&
                    ...
                    w[176..184] == request && (!NONCE_LINKED || (w[136..139]==[0,0,0] && w[139]==request[6])) && w[145..176].iter().all(|x| *x == 0) &&
...
// Keep existing panic source locations (and legacy raw images) byte-identical.
#[cfg(feature = "rp1-spi-retained-final-record")]
impl Rp1PcieTransport {
    pub fn log_spi_retained_final_result(&mut self) {
        crate::logln!("[RP1KEEPFINAL] wait addr=0x2000fc00 bytes={} polls=2000 interval_ms=20", SPI_FINAL_BYTES);
        match wait_spi_peer_final(self, || crate::timer::delay_millis(20)) {
            Ok(record) => {
                for (chunk, data) in record.chunks_exact(16).enumerate() {
                    crate::logln!(
                        "[RP1KEEPRESULT] addr=0x{:08x} data={:02x?}",
                        debug::MAILBOX_ADDR + chunk as u32 * 16,
                        data
                    );
                }
...
// Append only: preserve prior panic-source line numbers and raw images.
#[cfg(feature = "rp1-spi-deadline-final-record")]
impl Rp1PcieTransport {
    pub fn log_spi_deadline_final_result(&mut self) {
        const _: () = assert!(debug::MAILBOX_ADDR == 0x2000_fc00);
        const _: () = assert!(SPI_FINAL_BYTES == 160 * 4 && SPI_FINAL_BYTES / 16 == 40);
        const _: () = assert!(u32::from_le_bytes(SPI_FINAL_MAGIC) == 0x3243_3053);
        const _: () = assert!(debug::MAILBOX_ADDR + SPI_FINAL_BYTES as u32 - 1 == 0x2000_fe7f);
        crate::logln!("[RP1DLINEFINAL] wait addr=0x2000fc00 bytes={} polls=2000 interval_ms=20", SPI_FINAL_BYTES);
        match wait_spi_peer_final(self, || crate::timer::delay_millis(20)) {
            Ok(record) => {
                for (chunk, data) in record.chunks_exact(16).enumerate() {
                    crate::logln!(
                        "[RP1DLINERESULT] addr=0x{:08x} data={:02x?}",
                        debug::MAILBOX_ADDR + chunk as u32 * 16,
                        data
                    );
                }
                crate::logln!("[RP1DLINEFINAL] readout=complete header-check=match");
            }
            Err(reason) => crate::logln!("[RP1DLINEFINAL] failure={}", reason),
        }
    }
}

// Append only: preserve prior panic-source line numbers and raw images.
#[cfg(feature = "rp1-i2c-readonly-final-record")]
impl Rp1PcieTransport {
    pub fn log_i2c_readonly_final_result(&mut self) {
        const _: () = assert!(debug::MAILBOX_ADDR == 0x2000_fc00);
        const _: () = assert!(SPI_FINAL_BYTES == 40 * 4 && SPI_FINAL_BYTES / 16 == 10);
        const _: () = assert!(u32::from_le_bytes(SPI_FINAL_MAGIC) == 0x3152_3149);
        const _: () = assert!(debug::MAILBOX_ADDR + SPI_FINAL_BYTES as u32 - 1 == 0x2000_fc9f);
        crate::logln!("[RP1I2CROFINAL] wait addr=0x2000fc00 bytes={} polls=2000 interval_ms=20", SPI_FINAL_BYTES);
        match wait_spi_peer_final(self, || crate::timer::delay_millis(20)) {
            Ok(record) => {
                for (chunk, data) in record.chunks_exact(16).enumerate() {
                    crate::logln!(
                        "[RP1I2CRORESULT] addr=0x{:08x} data={:02x?}",
                        debug::MAILBOX_ADDR + chunk as u32 * 16,
                        data
                    );
                }
                crate::logln!("[RP1I2CROFINAL] readout=complete header-check=match");
            }
            Err(reason) => crate::logln!("[RP1I2CROFINAL] failure={}", reason),
        }
    }
}

// Append only: preserve prior panic-source line numbers and raw images.
#[cfg(feature = "rp1-i2c-stop-final-record")]
impl Rp1PcieTransport {
    pub fn log_i2c_stop_final_result(&mut self) {
        const _: () = assert!(debug::MAILBOX_ADDR == 0x2000_fc00);
        const _: () = assert!(SPI_FINAL_BYTES == 128 * 4 && SPI_FINAL_BYTES / 16 == 32);
        const _: () = assert!(u32::from_le_bytes(SPI_FINAL_MAGIC) == 0x3153_3149);
        const _: () = assert!(debug::MAILBOX_ADDR + SPI_FINAL_BYTES as u32 - 1 == 0x2000_fdff);
        crate::logln!("[RP1I2CSTOPFINAL] wait addr=0x2000fc00 bytes={} polls=2000 interval_ms=20", SPI_FINAL_BYTES);
        match wait_spi_peer_final(self, || crate::timer::delay_millis(20)) {
            Ok(record) => {
                for (chunk, data) in record.chunks_exact(16).enumerate() {
                    crate::logln!(
                        "[RP1I2CSTOPRESULT] addr=0x{:08x} data={:02x?}",
                        debug::MAILBOX_ADDR + chunk as u32 * 16,
                        data
                    );
                }
                crate::logln!("[RP1I2CSTOPFINAL] readout=complete header-check=match");
            }
            Err(reason) => crate::logln!("[RP1I2CSTOPFINAL] failure={}", reason),
        }
    }
}

// Append only: preserve prior panic-source line numbers and raw images.
#[cfg(feature = "rp1-i2c-read1-final-record")]
impl Rp1PcieTransport {
    pub fn log_i2c_read1_final_result(&mut self) {
        const _: () = assert!(debug::MAILBOX_ADDR == 0x2000_fc00);
        const _: () = assert!(SPI_FINAL_BYTES == 192 * 4 && SPI_FINAL_BYTES / 16 == 48);
        const _: () = assert!(u32::from_le_bytes(SPI_FINAL_MAGIC) == 0x3158_3149);
        const _: () = assert!(debug::MAILBOX_ADDR + SPI_FINAL_BYTES as u32 - 1 == 0x2000_feff);
        crate::logln!("[RP1I2CREAD1FINAL] wait addr=0x2000fc00 bytes={} polls=2000 interval_ms=20", SPI_FINAL_BYTES);
        match wait_spi_peer_final(self, || crate::timer::delay_millis(20)) {
            Ok(record) => {
                for (chunk, data) in record.chunks_exact(16).enumerate() {
                    crate::logln!(
                        "[RP1I2CREAD1RESULT] addr=0x{:08x} data={:02x?}",
                        debug::MAILBOX_ADDR + chunk as u32 * 16,
                        data
                    );
                }
                // Stable transport only, never an IRQ/byte/cleanup PASS verdict.
                crate::logln!("[RP1I2CREAD1FINAL] readout=complete header-check=match");
            }
